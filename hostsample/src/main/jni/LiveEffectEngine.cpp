...
static std::shared_ptr< LockFreeQueue<DWORD, 128> > midiEventsQueue;

LiveEffectEngine::LiveEffectEngine() {
    assert(mOutputChannelCount == mInputChannelCount);
    inputModule=new Module();
    effectModule=new Module();
    outputModule=new Module();

    midiEventsQueue = std::shared_ptr<LockFreeQueue<DWORD, 128> >( new LockFreeQueue<DWORD, 128>()  );
...
void LiveEffectEngine::setEngineOn(bool isOn){
    if (isOn != mIsEffectOn) {
        mIsEffectOn = isOn;
...
void LiveEffectEngine::openAllStreams() {
    ...
    openPlaybackStream();
    if(micEnabled) openRecordingStream();
    ...
    if (/*mRecordingStream && */mPlayStream) {
        if(micEnabled) startStream(mRecordingStream);
...
void LiveEffectEngine::openPlaybackStream() {
    ...
    mBufferInterleaved=new float[2*kBufferSize];

    for(int i=0;i<(2*kBufferSize);i++) {
        mBufferInterleaved[i] = 0;
    }
...
oboe::AudioStreamBuilder *LiveEffectEngine::setupPlaybackStreamParameters(
    ...
    builder->setCallback(this)
        ...
        ->setChannelCount(kNumCh)
        ->setFramesPerCallback(kBufferSize)
...
bool LiveEffectEngine::isModuleAlive(void *shm_ptr, int index, int instance_index) {
    if(effectModule->engineRef==shm_ptr && effectModule->instanceIndex==instance_index)
        return effectModule->statusError==false;
    else if(inputModule->engineRef==shm_ptr && inputModule->instanceIndex==instance_index)
        return inputModule->statusError==false;
    else if(outputModule->engineRef==shm_ptr && outputModule->instanceIndex==instance_index)
        return outputModule->statusError==false;
}

void LiveEffectEngine::DisconnectModule(void *shm_ptr, int index, int instance_index) {
    if(effectModule->engineRef==shm_ptr && effectModule->instanceIndex==instance_index)
...
bool LiveEffectEngine::AttachInput(bool enabled) {
    inputModule->connected = false;
    micEnabled=enabled;
...
bool LiveEffectEngine::AttachOutput(bool enabled) {
    outputModule->connected = false;
    speakersEnabled=enabled;
    return speakersEnabled;
}

void LiveEffectEngine::InitModule(void *shm_ptr, int index, int instance_index, bool isInput, bool isOutput) {
    ...
    module->engineRef=shm_ptr;
    module->moduleIndex=index;
    module->instanceIndex=instance_index;

    int connectedInputBuses[MaxNumBuses]={0};
    int connectedOutputBuses[MaxNumBuses]={0};
    if(isInput||!isOutput) connectedOutputBuses[0]=2;
    if(!isInput||isOutput) connectedInputBuses[0]=2;
    audioroute_initialize(shm_ptr, index, kBufferSize, kNumCh, kSampleRate, instance_index, connectedInputBuses, connectedOutputBuses);

    module->connected=true;
    module->statusError=0;
}
...
void GenerateSaw(int numFrames, float* out)
{
...
#define PHI 6.28
        ...
        float val=4*sin(i/((double)44100)*PHI*44100);

        LOGI("saw sample : %f", val);

            *out++=val;
        }
}

void LiveEffectEngine::SendMidiNoteOn(int note, bool noteOn) {

    DWORD nota;

    nota = nota & 0xffff00ff;
    nota |= ((note & 0xff) << 8);

    int velocity = noteOn ? 127 : 0;
    nota = nota & 0xff00ffff;
    nota |= ((velocity & 0xff) << 16);

    midiEventsQueue->push(nota);
}

#define MIDI_NOTE(x) (((x)&0xff00)>>8)
#define MIDI_VELOCITY(x) (((x)&0xff0000)>>16)

void LiveEffectEngine::FillMidiEvents() {

    numEvents=0;
    int size=midiEventsQueue->size();
    for(int i=0;i<midiEventsQueue->size();i++) {
        DWORD nota;
        midiEventsQueue->pop(nota);
        MusicEvent event;
        event.eventType=MIDI_VELOCITY(nota)==0 ? EventTypeNoteOff : EventTypeNoteOn;
        event.index=MIDI_NOTE(nota);
        event.value=0.8;
        event.deltaFrames=0;
        event.noteLengthFrames=0;
 ...
 */
static long long samplesStreamed=0;

oboe::DataCallbackResult LiveEffectEngine::onAudioReady(
    oboe::AudioStream *oboeStream, void *audioData, int32_t numFrames) {
    if(0==numFrames) return oboe::DataCallbackResult::Continue;
    assert(oboeStream == mPlayStream);

    AudiorouteTimeInfo timeInfo={0};
    timeInfo.bpm=120;
    timeInfo.timeSignatureNumerator=4;
    timeInfo.timeSignatureDenominator=4;
    timeInfo.sampleOffset=samplesStreamed;
    samplesStreamed+=numFrames;

    int32_t prevFrameRead = 0, framesRead = 0;
    if (mProcessedFrameCount < mSystemStartupFrames) {
        int32_t bytesPerFrame = mPlayStream->getChannelCount() *
        ...
        //INPUT
        if(inputModule->connected && !inputModule->statusError) {

            FillMidiEvents();
            inputModule->statusError=audioroute_process_interleaved(inputModule->engineRef, inputModule->moduleIndex, mBufferInterleaved, numFrames, kNumCh, kSampleRate,
                                           &events[0], numEvents, inputModule->instanceIndex, &timeInfo);
            ...
            oboe::convertPcm16ToFloat(static_cast<int16_t *>(audioData), mBufferInterleaved, 2*numFrames);
        }

        //EFFECT
        if(effectModule->connected && !effectModule->statusError) {

            effectModule->statusError = audioroute_process_interleaved(effectModule->engineRef,
                                                        effectModule->moduleIndex, mBufferInterleaved, numFrames, kNumCh,
                                                        kSampleRate, nullptr, 0,
                                                        effectModule->instanceIndex, &timeInfo);
        }

        //OUTPUT
        if(outputModule->connected && !outputModule->statusError) {
            outputModule->statusError=audioroute_process_interleaved(outputModule->engineRef, outputModule->moduleIndex, mBufferInterleaved, numFrames, kNumCh, kSampleRate,
                                                                     nullptr, 0, outputModule->instanceIndex, &timeInfo);
        }

        oboe::convertFloatToPcm16(mBufferInterleaved, static_cast<int16_t *>(audioData), 2*numFrames);

        if(!speakersEnabled || outputModule->connected) {
            //Mute speaker out
            int32_t bytesPerFrame = mPlayStream->getChannelCount() * oboeStream->getBytesPerSample();
            uint8_t *padPos = static_cast<uint8_t *>(audioData);
            memset(padPos, 0, (size_t) numFrames * bytesPerFrame);
        }
