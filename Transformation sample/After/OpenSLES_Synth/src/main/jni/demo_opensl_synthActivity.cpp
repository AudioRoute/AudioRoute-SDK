...
void process_func(void *context, int sample_rate, int framesPerBuffer,
    ...
    static int32_t tempEvents[MaxEvents];
    int numEventsCopied=0;
    for(int i=0; i<eventsNum; ++i)
    {
        tempEvents[numEventsCopied++]=ConvertMusicEventToRawMIDI(&(events[i]));
    }
    instanceData.process(output_buffer, numChannels>1 ? (output_buffer+framesPerBuffer) : NULL, framesPerBuffer, 1, sample_rate, numEventsCopied>0 ? tempEvents : NULL, numEventsCopied);
}
