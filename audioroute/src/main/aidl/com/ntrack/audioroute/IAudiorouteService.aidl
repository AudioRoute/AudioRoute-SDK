...
/**
 * Audioroute service interface, implemented by {@link Audioroute}.
 */
interface IAudiorouteService {
  
  /**
   * Registers a Audioroute client. Note that a client is not an audio module.
   * Rather, clients are Java objects that will be notified when the state
   * of the audioroute changes.
   */
  void registerClient(IAudiorouteClient client);
      
  /**
   * Unregisters a Audioroute client.
   */
  void unregisterClient(IAudiorouteClient client);
  ...
  /**
   * @return True if the Audioroute is currently rendering audio.
   */
  boolean isRunning();
  ...
  /**
   * @return The sample rate in Hz at which the Audioroute operates. This value is determined
  ...
  /**
   * @return The buffer size in frames at which the Audioroute operates. This value is determined
  ...
  /**
   * Creates a new audio module in the Audioroute service; for internal use mostly, to be called by
  ...
  /**
   * Deletes an audio module from the Audioroute service; for internal use mostly, to be called by
  ...
  /**
     * @return List of instantiable modules installed on the system
     */
  //Map getInstalledModules();

  /**
       * @return Instantiate module and return pointers to exchange buffers
       */
  //Map instantiateModule();
}
