...
static int smi_transmit(int fd) {
 ...
 //char *path = "127.0.0.1";
  int sock_fd = socket(addr.sin_family, SOCK_STREAM, 0);
    ...
    int n = 1;
    setsockopt(sock_fd, SOL_SOCKET, SO_REUSEADDR, &n, sizeof(n));
#else
  ...
  int sock_fd = socket(addr.sun_family, SOCK_DGRAM, 0);
#endif
  ...
  if (isSender) {
    ...
    *((int *) CMSG_DATA(CMSG_FIRSTHDR(&hdr))) = fd;
    ...
    if (bind(sock_fd, (struct sockaddr *) &addr,
      ...
      close(sock_fd);
        int error=errno;
      LOGW("Failed to bind socket: %d, %s", error, strerror(error));
      return -1;
    }
