...
public:
    ...
    void process(float *bufferL, float *bufferR, int framesPerBuffer, int stride, int sample_rate, MusicEvent *events, int eventsNum)
    {
        int currentEvent=0;
            ...
            for(; currentEvent<eventsNum; ++currentEvent)
            {
                if(events[currentEvent].deltaFrames>=i) {
                    if (EventTypeNoteOn==events[currentEvent].eventType) {
                        ...
                        voices[lastVoice].turn_on(events[currentEvent].index, sample_rate, waveform, linToGain(events[currentEvent].value));
                    }
                    else if (EventTypeNoteOff==events[currentEvent].eventType) {
                        for(int v=0; v<NumVoices; ++v) {
                            if(voices[v].is_on()&&voices[v].currentNote()==events[currentEvent].index)
                                voices[v].turn_off();
                        }
                    }
                } else break;
...
extern simplesynth_data simplesynthData;